...
void EditorOptionsLayer::onPreviewRotations(CCObject*) {
    ...
    if (setting().onPreviewRotations) {
        RotateSaws::beginRotations(LevelEditorLayerHook::get());
    }
    else {
        RotateSaws::stopRotations(LevelEditorLayerHook::get());
    }
}
...
bool EditorOptionsLayer::init() {
    ...
    auto onClose = CCMenuItemSpriteExtra::create(onCloseSpr, this, menu_selector(EditorOptionsLayer::onClose));
    ...
    auto onDurationLines = CCMenuItemToggler::create(toggleOff, toggleOn, this, menu_selector(EditorOptionsLayer::onDurationLines));
    onDurationLines->toggle(setting().onDurationLines);
    onDurationLines->setPosition(-160.f, 80.f);
    ...
    onDurationLinesLabel->setPosition(winSize.width / 2.f - 138.f, winSize.height / 2.f + 80.f);
    layer->addChild(onDurationLinesLabel);

    auto onExperimentalLayering = CCMenuItemToggler::create(toggleOff, toggleOn, this, menu_selector(EditorOptionsLayer::onExperimentalLayering));
    onExperimentalLayering->toggle(setting().onExperimentalLayering);
    onExperimentalLayering->setPosition(32.f, 80.f);
    ...
    onExperimentalLayeringLabel->setPosition(winSize.width / 2.f + 54.f, winSize.height / 2.f + 80.f);
    layer->addChild(onExperimentalLayeringLabel);

    auto onNewColorSelect = CCMenuItemToggler::create(toggleOff, toggleOn, this, menu_selector(EditorOptionsLayer::onNewColorSelect));
    onNewColorSelect->toggle(setting().onNewColorSelectMenu);
    onNewColorSelect->setPosition(-160.f, 40.f);
    menu->addChild(onNewColorSelect);
    auto onNewColorSelectLabel = CCLabelBMFont::create("New Color Select", "bigFont.fnt");
    onNewColorSelectLabel->limitLabelWidth(130.f, .5f, 0.f);
    onNewColorSelectLabel->setAnchorPoint({ 0.f, .5f });
    onNewColorSelectLabel->setPosition(winSize.width / 2.f - 138.f, winSize.height / 2.f + 40.f);
    layer->addChild(onNewColorSelectLabel);

    auto onPreviewRotations = CCMenuItemToggler::create(toggleOff, toggleOn, this, menu_selector(EditorOptionsLayer::onPreviewRotations));
    ...
    auto onShowClicks = CCMenuItemToggler::create(toggleOff, toggleOn, this, menu_selector(EditorOptionsLayer::onShowClicks));
    ...
    auto onUnusedObjects = CCMenuItemToggler::create(toggleOff, toggleOn, this, menu_selector(EditorOptionsLayer::onUnusedObjects));
