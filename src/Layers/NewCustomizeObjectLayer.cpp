#include "NewCustomizeObjectLayer.h"
...
SettingsColorObject* NewCustomizeObjectLayer::intToSettings(int colorID) {
...
bool NewCustomizeObjectLayer::init(GameObject* object, CCArray* objects) {
	...
	if (m_selectedObject == nullptr) {
		...
		CCARRAY_FOREACH_B_TYPE(m_selectedObjects, obj, GameObject) {
	...
	auto onCloseSpr = ButtonSprite::create("OK", 0, 0, 1.f, false, "goldFont.fnt", "GJ_button_01.png", 30.f);
	auto onClose = CCMenuItemSpriteExtra::create(onCloseSpr, this, menu_selector(NewCustomizeObjectLayer::onClose));
	...
	auto onEditColor = CCMenuItemSpriteExtra::create(m_selectedColorSprite, this, menu_selector(NewCustomizeObjectLayer::editColor));
	...
	auto toggleLiveColor = CCMenuItemToggler::create(toggleOff, toggleOn, this, menu_selector(NewCustomizeObjectLayer::toggleLiveColor));
	toggleLiveColor->toggle(setting().m_liveColorEnabled);
	toggleLiveColor->setPositionX(-96.f);
	menu->addChild(toggleLiveColor);

	toggleOff->setColor(gm->getGameVariable(PREVIEW_MODE) ? ccWHITE : ccGRAY);
	toggleOn->setColor(gm->getGameVariable(PREVIEW_MODE) ? ccWHITE : ccGRAY);
	toggleLiveColor->setEnabled(gm->getGameVariable(PREVIEW_MODE));
	liveColorsLabel->setColor(gm->getGameVariable(PREVIEW_MODE) ? ccWHITE : ccGRAY);
	...
	auto onDefault = CCMenuItemSpriteExtra::create(onDefaultSpr, this, menu_selector(NewCustomizeObjectLayer::onSelectColor));
	onDefault->setPosition(-60.f, 110.f);
	menu->addChild(onDefault, 0, 0);

	auto onLightBGSpr = ColorChannelSprite::create("LBG");
	auto onLightBG = CCMenuItemSpriteExtra::create(onLightBGSpr, this, menu_selector(NewCustomizeObjectLayer::onSelectColor));
	...
	auto onP1 = CCMenuItemSpriteExtra::create(onP1Spr, this, menu_selector(NewCustomizeObjectLayer::onSelectColor));
	...
	auto onP2 = CCMenuItemSpriteExtra::create(onP2Spr, this, menu_selector(NewCustomizeObjectLayer::onSelectColor));
	onP2->setPosition(-25.f, 75.f);
	menu->addChild(onP2, 0, 2);

	auto onWhiteSpr = ColorChannelSprite::create("W");
	auto onWhite = CCMenuItemSpriteExtra::create(onWhiteSpr, this, menu_selector(NewCustomizeObjectLayer::onSelectColor));
	onWhite->setPosition(-60.f, 40.f);
	menu->addChild(onWhite, 0, 9);

	auto on3DLSpr = ColorChannelSprite::create("3DL");
	auto on3DL = CCMenuItemSpriteExtra::create(on3DLSpr, this, menu_selector(NewCustomizeObjectLayer::onSelectColor));
	...
	auto onCol1 = CCMenuItemSpriteExtra::create(onCol1Spr, this, menu_selector(NewCustomizeObjectLayer::onSelectColor));
	onCol1->setPosition(25.f, 92.5f);
	menu->addChild(onCol1, 0, 3);
	onCol1Spr->setTag(3);

	auto onCol2Spr = ColorChannelSprite::create("2");
	auto onCol2 = CCMenuItemSpriteExtra::create(onCol2Spr, this, menu_selector(NewCustomizeObjectLayer::onSelectColor));
	onCol2->setPosition(60.f, 92.5f);
	menu->addChild(onCol2, 0, 4);
	onCol2Spr->setTag(4);

	auto onCol3Spr = ColorChannelSprite::create("3");
	auto onCol3 = CCMenuItemSpriteExtra::create(onCol3Spr, this, menu_selector(NewCustomizeObjectLayer::onSelectColor));
	onCol3->setPosition(25.f, 57.5f);
	menu->addChild(onCol3, 0, 6);
	onCol3Spr->setTag(6);

	auto onCol4Spr = ColorChannelSprite::create("4");
	auto onCol4 = CCMenuItemSpriteExtra::create(onCol4Spr, this, menu_selector(NewCustomizeObjectLayer::onSelectColor));
...
void NewCustomizeObjectLayer::colorSelectClosed(ColorSelectPopup* colorSelect) {
	auto editorLayer = LevelEditorLayerHook::get();
	if (editorLayer) {
		auto settings = editorLayer->m_levelSettings;
...
void NewCustomizeObjectLayer::updateSelectPosition() {
	if (m_selectSprite) {
        CCARRAY_FOREACH_B_TYPE(m_buttonsArray, btn, CCMenuItemSpriteExtra) {
...
void NewCustomizeObjectLayer::updateSelectedColorSprite() {
	if (m_selectedColorSprite) {
		...
		if (editorLayer) {
			...
			if (setting().m_liveColorEnabled && GameManager::sharedState()->getGameVariable(PREVIEW_MODE)) {
                CCARRAY_FOREACH_B_TYPE(m_colorSprites, colorSprite, ColorChannelSprite) {
		...
		if (colorSprite) {
			if (editorLayer) {
				if (setting().m_liveColorEnabled && GameManager::sharedState()->getGameVariable(PREVIEW_MODE)) {
					...
					case 3: colorSprite->updateValues(LevelEditorLayerHook::getColor01(), LevelEditorLayerHook::getColor01().blending); break;
					case 4: colorSprite->updateValues(LevelEditorLayerHook::getColor02(), LevelEditorLayerHook::getColor02().blending); break;
					case 5: colorSprite->updateValues(getLightBGColor(editorLayer->m_backgroundSprite->getColor(), GameManager::sharedState()->colorForIdx(GameManager::sharedState()->m_playerColor)), true); break;
					case 6: colorSprite->updateValues(LevelEditorLayerHook::getColor03(), LevelEditorLayerHook::getColor03().blending); break;
					case 7: colorSprite->updateValues(LevelEditorLayerHook::getColor04(), LevelEditorLayerHook::getColor04().blending); break;
					case 8: colorSprite->updateValues(LevelEditorLayerHook::getColor3DL(), LevelEditorLayerHook::getColor3DL().blending); break;
					default: break;
					}
				}
				else {
					...
					case 5: colorSprite->updateValues(getLightBGColor(editorLayer->m_levelSettings->m_backgroundColor->m_color, GameManager::sharedState()->colorForIdx(GameManager::sharedState()->m_playerColor)), true); break;
...
void NewCustomizeObjectLayer::onSelectColor(CCObject* sender) {
	...
	if (m_selectedObject == nullptr) {
        CCARRAY_FOREACH_B_TYPE(m_selectedObjects, object, GameObject) {
			if (object) {
				if (iVar5 == 0) {
					object->updateCustomColorMode(static_cast<GJCustomColorMode>(m_selectedColorIdx), false);
				}
				else {
		...
		m_selectedObject->updateCustomColorMode(static_cast<GJCustomColorMode>(m_selectedColorIdx), false);
	}
	else {
...
void NewCustomizeObjectLayer::editColor(CCObject*) {
	...
	if (colorObject) {
		auto colorSelectPopup = ColorSelectPopup::create(0, colorSelectCID, colorObject->m_playerColor, colorObject->m_blending);
...
void NewCustomizeObjectLayer::toggleLiveColor(CCObject* sender) {
	...
	if (!GameManager::sharedState()->getGameVariable(PREVIEW_MODE)) {
		toggle->toggle(true);
		FLAlertLayer::create("Error", "You need to turn on Preview Mode first.", "OK")->show();
		return;
	}
