...
bool IconPreviewPopup::init() {
	...
	auto onClose = gd::CCMenuItemSpriteExtra::create(onCloseSpr, this, menu_selector(IconPreviewPopup::onClose));
	...
	auto playerIcon = gd::SimplePlayer::create(gm->m_playerFrame);
	...
	playerIcon->m_special = gm->m_playerStreak;
	playerIcon->updateColors();
	layer->addChild(playerIcon);

	auto shipIcon = gd::SimplePlayer::create(gm->m_playerShip);
	shipIcon->updatePlayerFrame(gm->m_playerShip, gd::IconType::Ship);
	...
	shipIcon->m_special = gm->m_playerStreak;
	shipIcon->updateColors();
	layer->addChild(shipIcon);

	auto rollIcon = gd::SimplePlayer::create(gm->m_playerBall);
	rollIcon->updatePlayerFrame(gm->m_playerBall, gd::IconType::Ball);
	...
	rollIcon->m_special = gm->m_playerStreak;
	rollIcon->updateColors();
	layer->addChild(rollIcon);

	auto birdIcon = gd::SimplePlayer::create(gm->m_playerBird);
	birdIcon->updatePlayerFrame(gm->m_playerBird, gd::IconType::UFO);
	...
	birdIcon->m_special = gm->m_playerStreak;
	birdIcon->updateColors();
	layer->addChild(birdIcon);

	auto dartIcon = gd::SimplePlayer::create(setting().selectedDartIdx);
	dartIcon->updatePlayerFrame(setting().selectedDartIdx, gd::IconType::Wave);
	...
	dartIcon->m_special = gm->m_playerStreak;
